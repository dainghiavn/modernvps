...
main() {
    ...
    # Restart fail2ban sau nginx — setup_security() chạy TRƯỚC nginx
    # → /var/log/nginx/*.log chưa tồn tại lúc fail2ban khởi động lần đầu
    if systemctl is-enabled fail2ban &>/dev/null; then
        log "Restart fail2ban sau khi nginx đã start..."
        systemctl restart fail2ban 2>/dev/null \
            || warn "Fail2ban restart lần 2 thất bại — xem: journalctl -u fail2ban"
    fi
