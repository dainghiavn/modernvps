...
setup_modsecurity() {
    ...
    if [[ "$modsec_ready" == "false" && "$OS_FAMILY" == "debian" ]]; then
        ...
        # Cài libmodsecurity3 (thư viện) + nginx connector module
        # ondrej/nginx KHÔNG đi kèm ngx_http_modsecurity_module.so
        # → phải cài libnginx-mod-http-modsecurity nếu có, hoặc build từ source
        if [[ -n "$modsec_pkg" ]] \
            ...
            # libmodsecurity3 đã cài nhưng thiếu nginx connector .so
            # (ondrej/nginx không bundle module này) → cần build từ source
    ...
    # Trường hợp ondrej/nginx: libmodsecurity3 cài được nhưng không có .so connector
    if [[ "$modsec_ready" == "false" && "$OS_FAMILY" == "debian" ]]; then
