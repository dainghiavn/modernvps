...
EOF
    ...
    # Fix: nginx log files phải tồn tại TRƯỚC khi fail2ban start
    # Fail2ban crash với "Have not found any log file" nếu logpath không tồn tại
    # setup_security() chạy TRƯỚC install_nginx_stack() → log chưa có
    # Giải pháp: tạo log files trống để fail2ban start được, nginx sẽ ghi sau
    if [[ "$SERVER_TYPE" == "web" ]]; then
        mkdir -p /var/log/nginx
        for _log in /var/log/nginx/error.log /var/log/nginx/access.log; do
            [[ ! -f "$_log" ]] && touch "$_log" && chmod 640 "$_log"                 && chown www-data:adm "$_log" 2>/dev/null || true
        done
    fi
